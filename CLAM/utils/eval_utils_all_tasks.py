...
def eval(dataset, args, ckpt_path, dataset_names):
    model = initiate_model(args, ckpt_path)
    
    print('Init Loaders')
    loader = get_simple_loader(dataset)
    patient_results, test_error, auc, f1, acc, precision, recall, kw, df, _ = summary(model, loader, args, dataset_names)
...
import time
def summary(model, loader, args, dataset_names):
    ...
    if len(np.unique(all_labels)) == 1:
        ...
        f = open('csv_results.csv', 'w')
        f.write('dataset,acc,f1,recall,precision\n')
        ...
        dataset_names_unique = sorted(list(set(dataset_names)))
        dict_results = {i:dict() for i in dataset_names_unique}
        for key in dict_results:
            dict_results[key] = dict(labels=list(), preds=list())
        
        for data_name, pred, label in zip(dataset_names, all_preds, all_labels):
            dict_results[data_name]['labels'].append(label)
            dict_results[data_name]['preds'].append(pred)

        for data_name in dict_results:
            print('-'*20)
            print("Results of ", data_name)
            f1 = f1_score(dict_results[data_name]['labels'], dict_results[data_name]['preds'], average='macro')
            acc = accuracy_score(dict_results[data_name]['labels'], dict_results[data_name]['preds'])
            precision = precision_score(dict_results[data_name]['labels'], dict_results[data_name]['preds'], average='macro')
            recall = recall_score(dict_results[data_name]['labels'], dict_results[data_name]['preds'], average='macro')
            kw = cohen_kappa_score(dict_results[data_name]['labels'], dict_results[data_name]['preds'], weights='quadratic')
            ...
            f.write(','.join([data_name, str(acc), str(f1), str(recall), str(precision)]) + '\n')
