...
from dataset.utils import prepare_dataset, prepare_dataset_online
...
class MyHandler(object):
    ...
    def exec_test_online(self, overlap=None):
        ...
        slide_ids = [os.path.basename(slide_id).split('.')[0] for slide_id in glob.glob(self.cfg['path_patch'] + '/*.pt')]
    ...
    def _update_disc(self, i_batch, xs, ys, mode='wlabel', label_visible_mask=None):
        ...
        print('train_batch/netD/Loss_D', dis_loss.item())
        print('train_batch/netD/D_real', 0.0 if bp_collector['real'] is None else bp_collector['real'].mean().item())
        print('train_batch/netD/D_fake', bp_collector['fake'].mean().item())
    ...
    def _update_gen(self, i_batch, xs, ys, mode='wlabel', label_visible_mask=None):
        ...
        print('train_batch/netG/Loss_G_fake', gen_loss.item()) 
        print('train_batch/netG/Loss_G_time', t_reg_loss.item())
        print('train_batch/netG/Loss_G_total', gen_total_loss.item())
        print('train_batch/netG/D_fake_avg', sum(f_fake_list) / len(f_fake_list))
